...
public class SPHManagerSingleThread : MonoBehaviour
{
    ...
    private void InitSPH()
    {
        ...
        for (int i = 0; i < amount; i++)
        {
            ...
            float x = (i % rowSize) + Random.Range(-0.1f, 0.1f) + character0Prefab.transform.position.x;
            float y = 2 + (float)((i / rowSize) / rowSize) * 1.1f + character0Prefab.transform.position.y;
            float z = ((i / rowSize) % rowSize) + Random.Range(-0.1f, 0.1f) + character0Prefab.transform.position.z;
